...
def plot_hotspot_area(y_true: np.ndarray, y_pred: np.ndarray, t_idx: list, threshold: int):
    ...
    """
    mean_error = losses.calculate_avg_sensitivity(
        y_true, y_pred, t_idx, threshold
    )
...
def plot_hotspot_temperature(y_true: np.ndarray, y_pred: np.ndarray, t_idx: list, threshold: int):
    ...
    # Add labels and title
    plt.title(
        r"Ave. Hotspot Temperature ($T_{hs}$)", fontsize=14, pad=15
    )
    # x-axis: time in nanoseconds
    plt.xlabel(r"t ($ns$)", fontsize=12)
...
def plot_hotspot_area_dot(y_true: np.ndarray, y_pred: np.ndarray, t_idx: list, threshold: int): 
    ...
    gt_area_error1_deriv = mean_error.get('Ground_truth').get('Area_gradient').get('error1')
    gt_area_error2_deriv = mean_error.get('Ground_truth').get('Area_gradient').get('error2')
    ...
    plt.fill_between(t_idx[1:], gt_area_error1_deriv, gt_area_error2_deriv, color="blue", alpha=0.2)
    plt.fill_between(t_idx[1:], area_error1_deriv, area_error2_deriv, color="red", alpha=0.2)
...
def plot_hotspot_temp_dot(y_true: np.ndarray, y_pred: np.ndarray, t_idx: list, threshold: int): 
    """plot of the average hotspot area rate of change used to show predicted growth
    Args:
        y_true (np.ndarray): true values for temp/pressure found in input dataset
        y_pred (np.ndarray): model predicted values for temp/press
    ...
    temp_error1_deriv = mean_error.get('Prediction').get('Temperature_gradient').get('error1')
    temp_error2_deriv = mean_error.get('Prediction').get('Temperature_gradient').get('error2')
    gt_temp_error1_deriv = mean_error.get('Ground_truth').get('Temperature_gradient').get('error1')
    gt_temp_error2_deriv = mean_error.get('Ground_truth').get('Temperature_gradient').get('error2')
    ...
    plt.fill_between(t_idx[1:], gt_temp_error1_deriv, gt_temp_error2_deriv, color="blue", alpha=0.2)
    plt.fill_between(t_idx[1:], temp_error1_deriv, temp_error2_deriv, color="red", alpha=0.2)
...
def plot_saliency(y_pred: np.ndarray, cn: int, ts: int):
