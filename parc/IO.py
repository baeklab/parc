...
def parseData(file_name: str, input_img_size: int, case_numbers: int, time_steps: int, del_t: int) -> np.ndarray:
    ...
    output_data = np.zeros( (case_numbers,input_img_size,input_img_size,time_steps,4) ) #output dimension 5 shape: [0:T,1:P,2:T_dot,3:P_dot]
    ...
    for case_idx in range(1,case_numbers + 1):
        ...
        original_img = './' + file_name + '/microstructures/data_'+str(format(case_idx,'02d'))+'.pgm'
        ...
        for time_idx in range(0,time_steps):
            ...
            if time_idx ==0:
                temp = np.full((input_img_size,input_img_size),300.)
                ...
                temperature_name =  './data/raw/temperatures/data_'+str(format(case_idx,'02d'))+'/Temp_'+str(format(time_idx,'02d'))+'.txt'
                ...
                pressure_name =  './data/raw/pressures/data_'+str(format(case_idx,'02d'))+'/pres_'+str(format(time_idx,'02d'))+'.txt'
                ...
                Tdot = output_data[case_idx-1,:,:,time_idx,0] - output_data[case_idx-1,:,:,time_idx-1,0]
                Pdot = output_data[case_idx-1,:,:,time_idx,1] - output_data[case_idx-1,:,:,time_idx-1,1]
                Tdot = Tdot/del_t
                Pdot = Pdot/del_t
    ...
    for channel in range(0,4):
        ...
        output_data[:,:,:,:,channel] = ((output_data[:,:,:,:,channel]-norm_min)/(norm_max-norm_min))
        output_data[:,:,:,:,channel] = (output_data[:,:,:,:,channel] * 2.0)-1.0
        print('max and min of channel ' + str(channel) + ' are: ',norm_max,norm_min)

    #Normalize input data to range [-1,1]
    microstructure_data[:,:,:,0] = (microstructure_data[:,:,:,0]-
        np.amin(microstructure_data[:,:,:,0]))/(np.amax(microstructure_data[:,:,:,0])-
        np.amin(microstructure_data[:,:,:,0]))
    microstructure_data[:,:,:,1] = (microstructure_data[:,:,:,1]-
        np.amin(microstructure_data[:,:,:,1]))/(np.amax(microstructure_data[:,:,:,1])-
...
def splitData(data_in: np.ndarray, output_data: np.ndarray, splits: list) -> np.ndarray:
